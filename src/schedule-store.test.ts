...
import { createScheduleStore, validateScheduleFile, validateOccurrenceFile } from "./schedule-store.js";
...
function makeValidSchedule(id: string, overrides?: Partial<import("./schedule-store.js").ScheduleFile>): import("./schedule-store.js").ScheduleFile {
...
describe("validateScheduleFile", () => {
  ...
  it("rejects wrong version", () => {
    const schedule = makeValidSchedule("sch_20260525_120000_test", { version: 2 as 1 });
    expect(validateScheduleFile(schedule)).toContain("version must be 1");
  });

  it("rejects wrong kind", () => {
    const schedule = makeValidSchedule("sch_20260525_120000_test", { kind: "task_group" as "schedule" });
  ...
  it("rejects mismatched directory id", () => {
    const schedule = makeValidSchedule("sch_20260525_120000_test");
    expect(validateScheduleFile(schedule, "sch_20260525_120000_other")).toContain(
      "directory id and content id do not match",
    );
  });

  it("rejects missing title", () => {
    const schedule = makeValidSchedule("sch_20260525_120000_test", { title: "" });
    expect(validateScheduleFile(schedule)).toContain("title is required");
  });

  it("rejects invalid status", () => {
    const schedule = makeValidSchedule("sch_20260525_120000_test", { status: "paused" as "active" });
    expect(validateScheduleFile(schedule)).toContain("status is invalid");
  });

  it("rejects relative projectRoot", () => {
    const schedule = makeValidSchedule("sch_20260525_120000_test", { projectRoot: "relative/path" });
    expect(validateScheduleFile(schedule)).toContain("projectRoot must be an absolute path");
  });

  it("rejects cwd outside projectRoot", () => {
    const schedule = makeValidSchedule("sch_20260525_120000_test", { cwd: "/other/path" });
    expect(validateScheduleFile(schedule)).toContain("cwd must be within projectRoot");
  });

  it("rejects invalid timezone", () => {
    const schedule = makeValidSchedule("sch_20260525_120000_test", { timezone: "Mars/Phobos" });
    expect(validateScheduleFile(schedule)).toContain("timezone is invalid");
  });

  it("rejects missing intent.prompt", () => {
    const schedule = makeValidSchedule("sch_20260525_120000_test", { intent: { prompt: "" } });
    expect(validateScheduleFile(schedule)).toContain("intent.prompt is required");
  });
  ...
  it("rejects once timing without runAt", () => {
    ...
    expect(validateScheduleFile(schedule)).toContain("once timing requires runAt");
  });
  ...
  it("rejects endsAt earlier than startsAt", () => {
    ...
    expect(validateScheduleFile(schedule)).toContain("endsAt must be later than startsAt");
  });
  ...
  it("rejects command executor without command", () => {
    ...
    expect(validateScheduleFile(schedule)).toContain("execution.command is required when executor is command");
  });
  ...
  it("rejects invalid overlapPolicy", () => {
    ...
    expect(validateScheduleFile(schedule)).toContain('execution.overlapPolicy must be "allow" or "skip"');
  });

  it("rejects negative triggeredCount", () => {
    const schedule = makeValidSchedule("sch_20260525_120000_test", { triggeredCount: -1 });
    expect(validateScheduleFile(schedule)).toContain("triggeredCount must be a non-negative number");
  });
});
...
describe("validateOccurrenceFile", () => {
  ...
  it("rejects wrong kind", () => {
    ...
    expect(validateOccurrenceFile(occ)).toContain('kind must be "schedule_occurrence"');
  });
...
describe("createScheduleStore", () => {
  ...
  it("lists schedules with default filters", () => {
    store.save(makeValidSchedule("sch_20260525_120000_a", { status: "active" }));
    store.save(makeValidSchedule("sch_20260525_120000_b", { status: "cancelled" }));
    store.save(makeValidSchedule("sch_20260525_120000_c", { status: "archived" }));
  ...
  it("saves and reads an occurrence", () => {
    ...
    const read = store.readOccurrence("sch_20260525_120000_test", "occ_20260525_120000_x7a9");
    expect(read).not.toBeNull();
  ...
  it("lists occurrences sorted by scheduledAt desc", () => {
    ...
    store.saveOccurrence(
      makeValidOccurrence("sch_20260525_120000_test", "occ_20260525_100000_x7a9", {
        scheduledAt: "2026-05-25T10:00:00Z",
      }),
    );
    store.saveOccurrence(
      makeValidOccurrence("sch_20260525_120000_test", "occ_20260525_120000_x7a9", {
  ...
  it("limits occurrences", () => {
    ...
    for (let i = 0; i < 5; i++) {
      store.saveOccurrence(
        makeValidOccurrence("sch_20260525_120000_test", `occ_20260525_${String(i).padStart(2, "0")}0000_x7a9`, {
  ...
  it("filters by projectRoot", () => {
    store.save(makeValidSchedule("sch_20260525_120000_a", { projectRoot: "/tmp/project-a", cwd: "/tmp/project-a" }));
    store.save(makeValidSchedule("sch_20260525_120000_b", { projectRoot: "/tmp/project-b", cwd: "/tmp/project-b" }));
