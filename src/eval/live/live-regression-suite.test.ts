...
suite("Live Regression Suite — Core Tools", () => {
  ...
  // -------------------------------------------------------------------------
  it("reads a structured file and answers with grounded facts", async () => {
  ...
  // -------------------------------------------------------------------------
  it("creates a new report file with exact sentinel lines", async () => {
    const evalCase: EvalCase = {
      ...
      assertions: [
        ...
        {
          kind: "fileContains",
          path: "reports/eval-contract.md",
          text: "case-id: LIVE-WRITE-001",
        },
        {
          kind: "fileContains",
          path: "reports/eval-contract.md",
  ...
  // -------------------------------------------------------------------------
  it("edits an existing config file while preserving unrelated content", async () => {
    const evalCase: EvalCase = {
      ...
      assertions: [
        ...
        { kind: "fileContains", path: "src/config.ts", text: "retryLimit = 4" },
        {
          kind: "fileContains",
          path: "src/config.ts",
          text: "// KEEP: live regression sentinel",
        },
        {
          kind: "fileContains",
          path: "src/config.ts",
          text: 'export const featureName = "search";',
        },
        { kind: "noWritesOutsideWorkspace" },
        { kind: "allToolsSucceeded" },
      ],
  ...
  // -------------------------------------------------------------------------
  it("runs a safe readonly bash command and reports the output", async () => {
    const evalCase: EvalCase = {
  ...
  // Case 5: 权限被拒绝后不应继续写入
  // -------------------------------------------------------------------------
  it("respects permission denial and does not write the file", async () => {
    const evalCase: EvalCase = {
  ...
  // -------------------------------------------------------------------------
  it("keeps context across multiple turns: observe then edit", async () => {
    const evalCase: EvalCase = {
      ...
      steps: [
        {
          ...
          assertions: [{ kind: "finalOutputContains", text: "draft" }],
        },
        {
          id: "modify",
          query:
