# PDD-18: Full-tools Live E2E 与 Replay/Live/Judge 回归设计

## 审阅结论

PDD-17 已经把 Eval Core、当前项目 in-process driver、deterministic suite、replay、live smoke、judge/report 以及第一轮 core-tools live regression 打通。
...
本 PDD 是 PDD-17 的后续设计，不替代 PDD-17。PDD-17 仍是 Eval Core 与 case schema 的来源；PDD-18 只负责把当前项目的复杂系统能力接入 live E2E。
...
本文件对应 Replay、Live Smoke、Judge/Report、Live Regression 和 Full-tools Live E2E。Live 类测试默认需要显式环境变量开启，避免普通 `npm test` 依赖真实模型、网络或外部状态。
