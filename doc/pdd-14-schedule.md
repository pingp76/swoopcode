...
当前实现裁剪（2026-05 第一轮重构）：

- `ScheduleFile.outputPolicy.saveRawOutput` 仍保留在存储结构中，但 `run_schedule_create` 暂不向 LLM 暴露 `save_raw_output` 参数，当前固定保存为 `true`。
- `ScheduleFile.outputPolicy.linkedTaskUpdate` 仍保留在存储结构中，但 `run_schedule_create` 暂不向 LLM 暴露 `linked_task_update` 参数，当前固定保存为 `"never"`。
- 这两个字段属于后续 OutputStore / Task 集成章节的设计预留，不应在当前版本的 tool description 中承诺已经实现。

当前实现裁剪（2026-05 第三轮重构）：

- `ScheduleManager` 已接入共享 `ExecutionPolicy`：创建 schedule 时校验 resources，触发 command occurrence 前按 `schedule.execution.permissionProfile` 校验 command。
- `run_schedule_create` 的 tool schema 仍只公开 readonly 语义，不允许 LLM 创建 `ci` 或 `workspace_write` schedule；`ci` 只用于兼容旧文件和后续章节的内部 profile 语义。
- 旧文件中的 `ci` command 可以在触发时按 ci profile 允许 `npm run build` 等 CI 命令；`workspace_write` 仍是 reserved，创建或触发都会失败。
- Schedule 仍只负责保存定时意图和 occurrence 审计，真正运行生命周期继续由 PDD13 Async Run 管理。

当前实现裁剪（2026-05 第四轮重构）：

- Schedule occurrence 已新增 `outputId` 字段，用于保存 Async Run 通过 OutputStore 登记后的输出句柄。
- `outputRef` 旧字段仍保留，用于兼容已有 occurrence 文件中的旧绝对路径引用；读取旧文件时不会自动把 `outputRef` 注册成新的可读 handle。
- Schedule notification 和 tool 展示优先给出 `output_id`，让 LLM 使用 `run_output_read` 读取完整输出。
- `saveRawOutput=false` 仍未实现；当前策略仍是保存 Async Run 输出，只是读取入口从裸路径升级为 OutputStore handle。
...
- `orphaned` 表示 occurrence 已经进入 `running`，但 Async Run 是 session-local，agent 进程重启后无法恢复该次执行实例。
...
6. 先把当前项目中遗留的 `running` occurrence 收敛为 `orphaned`，不尝试恢复旧 Async Run。
7. 检查进程离线期间是否有 missed occurrence。
8. 对 missed occurrence 只记录最近一次，不补跑历史所有 occurrence。
9. 将 orphaned / missed notification 放入 schedule notification queue。
...
当前实现边界（2026-05 第一轮重构）：

- Schedule 物理存储在 `<agentHome>/schedules`，但 tool/CLI/manager 默认只操作当前 `projectRoot` 的 schedule。
- `run_schedule_list` 默认只返回当前项目 summary；只有显式 `current_project_only: false` 才允许跨项目读取 summary。
- `run_schedule_read`、`run_schedule_cancel`、`run_schedule_delete`、`run_schedule_occurrence_list` 不跨项目操作。
- 当前 `run_schedule_create` 只暴露已实现字段：`notify_llm` 和 `summary_prompt` 可配置；`save_raw_output`、`linked_task_update`、`workspace_write` 不暴露给 LLM。
- `permission_profile` 当前只公开 `readonly` 语义；`ci` 只在 `ExecutionPolicy` 中有内部/兼容语义，`workspace_write` 仍是后续章节预留且当前会被拒绝。

当前实现边界（2026-05 第三轮重构）：

- Schedule trigger command preflight 不再维护自己的命令白名单，而是调用共享 `ExecutionPolicy.validateCommand({ profile: schedule.execution.permissionProfile })`。
- Schedule resources 由共享 `ExecutionPolicy.validateResources()` 校验；readonly/ci 下 `writePaths` 必须为空，`readPaths` 必须在 `projectRoot` 内。
- `workspace_write` profile 已被显式标记为 reserved，避免旧文件或手写数据绕过当前章节的只读执行边界。
...
`executor: "command"` 必须复用或扩展 PDD13 Async Command Policy。当前实现已经改为复用共享 `ExecutionPolicy`，Schedule、Async Run 和 async/scoped subagent 看到的是同一套 readonly/ci 命令语义。
...
- `schedule.json` 使用共享 `atomicWriteJsonFile()` 原子写入。
- occurrence 文件也使用共享 `atomicWriteJsonFile()` 原子写入。
- reader 和 writer 校验规则必须对称。
- 文件名和内容 id 必须匹配。

实现备注（2026-05 Runtime Hardening Round A）：

- ScheduleStore 的 `schedule.json`、occurrence 文件和派生 `index.json` 已接入共享原子写入工具。
- 本轮没有实现 occurrence retention；历史 occurrence 的裁剪仍属于 Runtime Hardening 后续 Round D/E。
...
Schedule 原始设计保留了规则、occurrence、missed run、overlap policy 与 Async Run 触发关系。后续重构重点修复了跨模块边界：

1. Schedule 存在全局 `agentHome/schedules`，但默认只能加载和触发当前 `projectRoot` 的 active schedule，避免一个项目进程误触发另一个项目的计划任务。
2. 进程重启后，遗留的 running occurrence 会收敛为 orphaned/failed 等可解释终态，不允许 durable occurrence 永久悬挂。
3. Schedule tool schema 只承诺当前真实支持的能力；未实现的 workspace_write、linked_task_update、save_raw_output=false 不再作为可用能力暗示给 LLM。
4. Schedule 创建和触发路径共用 `ExecutionPolicy`，无人值守执行不走交互式 ask。
5. occurrence 输出优先保存 `outputId`，读取路径与 Async Run、Compressor 共用 `OutputStore` handle 语义。
