# PDD-17: Agent 集成测试与 Eval Runner 框架设计
...
因此 PDD-17 不应该先做真实 LLM judge，也不应该先把 runner 写死到当前 Agent，而应该先做一个稳定的中立 harness：
...
本文件对应 Eval 基础框架：Eval Core、deterministic suite、真实 core tools、CLI driver。当前实现还支持 trace writer 和 in-process learn-claude-code driver，适合验证“同一输入、同一脚本 LLM、同一工具结果”下的确定性行为。
