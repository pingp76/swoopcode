...
Async Run 原始设计保留了非阻塞运行实例、状态查询、输出读取和 notification 注入的完整方案。后续重构对当前实现做了几处边界收紧：

1. Async Run 的 command/subagent executor 共享 `ExecutionPolicy`，后台运行默认不能写项目文件，也不会注册 write/edit/task/todo/memory/subagent/async 等高风险工具给 async subagent。
2. 完成回调有清晰注册路径，避免 Schedule 或其他模块依赖 AsyncRunManager 的私有字段。
3. 输出登记到 `OutputStore`，新提示优先使用 `run_output_read(output_id)`；`run_async_output_read(run_id)` 保留为教学兼容入口。
4. running 状态要能在超时、late result、重复 finish 等分支中幂等收敛，不能重复 notification，也不能让 running count 泄漏。
5. 前台写入与后台 read claim 冲突时，Agent 会在执行前阻止写工具，避免并发读写同一片工作区。
