...
历史仓库中的 `pdd2.md` 与 `pdd1.md` 是同一份基础循环文档，并没有独立展开工具系统。为了避免学生把重复文档误解成第二章设计，本文件保留新整理后的工具章节，并在这里补足当前实现的关键细节。

当前工具层的主线是：`tools/registry.ts` 负责稳定注册工具定义与 executor，`tools/bash.ts` 提供 `run_bash`，`tools/files.ts` 提供 `run_read`、`run_write`、`run_edit` 和更安全的 `run_edit_exact`，`tools/output.ts` 提供 `run_output_read`。普通工具返回 `ToolResult`，由 Agent Loop 转成 LLM 可见的 `tool_result` 消息。

重构后新增的两个重要边界是：第一，大输出不再把物理路径直接暴露给模型，而是登记到 `OutputStore` 后返回 `output_id`，模型只能通过 `run_output_read(output_id)` 读回；第二，`run_edit_exact` 要求 oldText 精确唯一匹配，避免早期 `run_edit` 那种“替换到非预期位置”的常见错误。
