...
本文件保留两份原始设计：原 `pdd1-2.md` 说明轻量 Provider Profile 如何接入 Kimi Code CN 等 OpenAI-compatible 服务；原 `pdd21.md` 在此基础上进一步设计 Foundation Model Profile 与 Agent Runtime Policy。两者合并后，学生可以从“先把 endpoint/model/key 收束起来”一路看到“再把不同基座模型的上下文、thinking、reasoning、cache 行为收束成运行策略”。

---

## 原始设计 A：LLM Provider Profile 抽象层与 Kimi Code CN 接入设计
...
如果继续把这些差异塞进 `.env` 和 `llm.ts` 注释里，后续 agent 很容易在实现其他厂商时复制粘贴出多套分支。Provider Profile 前置设计 的目标是给出一个足够小但清晰的扩展点。
...
1. Provider Profile 前置设计 明确 provider profile 不进入 system prompt。
...
# PDD-16: 基座模型能力画像与 Agent Runtime Policy 抽象层设计

## 审阅结论

本阶段应该做一个 **基座模型能力画像 + Runtime Policy 解析层**，而不是做一个完整的多模型网关。

当前项目已经有 Provider Profile 前置设计 引入的 `LLM Provider Profile`：
...
如果这些差异散落在 `agent.ts`、`llm.ts`、`compressor.ts` 和 prompt 文本里，项目会很快失去教学清晰度。PDD-16 的目标是把这些差异收束成一个小而明确的运行策略层。
...
Profile Registry 的确有过时风险。PDD-16 的 registry 不是“永远正确的模型百科”，而是一个 **保守的、可审计的本地能力画像表**。实现时必须按以下规则控制维护成本和失效风险。
...
`src/llm.ts` 当前只返回 `content/toolCalls/finishReason`。PDD-16 需要扩展为：
...
- 当前 `kimi_code_cn` 已有 streaming 偏好，PDD-16 不应删除旧行为。
...
PDD-16 不能只把 thinking、reasoning effort 和 context budget 放在环境变量里。用户调试模型行为时，需要在 REPL 中快速查询和调整当前策略。
...
未来如果要支持运行中 protocol 对比，应另开设计：它需要 history 转换、tool schema 快照、transcript 分叉和 cache 前缀隔离，不属于 PDD-16 第一版。
...
当前 `agent.ts` 很可能根据 `LLMResponse.content/toolCalls` 重新构造 assistant 消息。PDD-16 后应改为：
...
PDD-16 的稳定大上下文装载器不是替代这些机制，而是在它们后面新增一个 **Stable Project Context Pack**：

```text
system prompt snapshot     # 现有机制，仍然只设置一次
tools definitions          # 现有机制，顺序稳定
stable project context     # PDD-16 新增：repo map / docs / pinned files
working set context        # PDD-16 新增：当前任务相关文件
conversation history       # 现有 history + compressor 管线
dynamic evidence pack      # PDD-16 新增：diff / test failure / output_id
...
Stable Project Context Pack
  PDD-16 新增机制。保存 repo map、项目文档、pinned files 等项目材料。

Evidence Pack
  PDD-16 新增机制。保存动态观察结果，靠近当前 query，不进入稳定前缀。
```
...
5. `tools/subagent.ts` 与 `async-runs.ts`
   ...
   - PDD-16 后还要复用创建时的 stable context snapshot。
...
1. 用户明确说“按 PDD-16 实现”，则 `doc/pdd-16-model-policy.md` 进入 working set。
...
```text
/c 加 src/agent.ts
/c 加 doc/pdd-16-model-policy.md
/c 删 src/agent.ts
/c 列
```
...
PDD-16 已经实现了基座模型画像、RuntimePolicy、ContextBudget 和 StableContextManager 的第一版。当前 stable context loader 能生成 repo map、装载 pinned files 和 `doc/summary.md`，但文件选择仍偏保守。
...
本设计是 PDD-16 的补充，目标是让 stable context loader 从“JS 代码仓库排序器”升级为“通用项目内容重要性排序器”。
...
4. 与 PDD-16 的 StableContextManager 集成，但不把文件选择逻辑写进 `agent.ts`。
...
PDD-16 的 `RuntimePolicy.contextLoading` 继续负责预算。通用内容重要性排序层只决定“预算内放什么”。
...
当前 `cache-debug.ts` 是本地稳定前缀 hash，不是真实 provider cache hit rate。PDD-16 后新增两个概念：
...
实现 PDD-16 前，coding agent 必须：

1. 重新读取本文件。
2. 逐条提取 checklist。
3. 读取 `doc/summary.md`。
4. 读取 `doc/pdd-16-model-policy.md`。
...
当前实现同时保留 Provider Profile 和 Foundation Model Profile 两层：`llm-providers.ts` 解决 provider/baseURL/model/apiKey/capability 的启动期解析；`foundation-models.ts`、`runtime-policy.ts`、`runtime-policy-store.ts` 负责把具体基座模型差异转成 Agent Runtime Policy。

模型适配不进入 system prompt，不让主循环写 `if model === ...` 分支。`llm-adapter.ts` 负责 OpenAI Chat Completions 请求构建、streaming 聚合、reasoning 回放和 provider 方言归一；`context-budget.ts`、`stable-context.ts`、`context-ranking.ts` 负责把模型上下文能力变成稳定上下文包和动态 working set。
