...
消息压缩章节的原始设计保留三层压缩思路。当前实现额外修正了消息管线不变量：`normalizeMessages()` 是 clone-based 纯函数，不修改输入数组或输入 message 对象；assistant 的 `tool_calls` 与对应 `tool_result` 会保持连续 block；孤立 tool result 不进入最终 LLM 输入；带 tool_calls 的 assistant 不参与普通 assistant 合并。

压缩后的大输出现在优先进入 `OutputStore`，返回稳定 `output_id` 和 `run_output_read` 指引，而不是让 LLM 看到裸文件路径。`MessageBlock` 同时携带清理后的 timeline metadata，使 `Compressor.decayOldBlocks()` 可以优先按全 session `loopIndex` 判断消息年龄，旧 `round` 只作为兼容 fallback。
